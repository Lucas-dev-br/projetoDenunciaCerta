...
comentariosRouter.get('/', async (req, res) => {
  try {
    // Consulta SQL para obter todos os comentários do banco de dados
    ...
    res.status(500).json({ error: 'Erro ao obter todos os comentários.' });
  }
});

// Rota para adicionar um comentário sem a necessidade de uma denúncia específica
comentariosRouter.post('/enviar', autenticacaoMiddleware, async (req, res) => {
  const { titulo, descricao } = req.body;
  const idUsuario = req.session.usuario.id;

  try {
    const result = await pool.query(
      'INSERT INTO comentarios (id_usuario, titulo, descricao) VALUES ($1, $2, $3) RETURNING *',
