...
app.get('/', (req, res) => {
  res.render('index'); // Renderiza o arquivo EJS (index.ejs)
});
...
app.use('/comentarios', cors({credentials: false}), comentariosRoutes);
app.use('/status-count', statusCountRoutes);
app.post('/login', cors({credentials: false}), authController.loginUser);
app.post('/registrar',cors({credentials: false}), (req, res) => {

  registrarController.cadastro(req, res, pool);
});

app.get('/teste',  cors({credentials: false}), (req, res) => {
  res.json({ mensagem: 'Conexão bem-sucedida!' });
});

app.use('/', cors({credentials: false}), webRouter);
